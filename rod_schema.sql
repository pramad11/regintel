...
-- Regulatory Obligation Database (ROD) — PostgreSQL Schema v1.0
-- =============================================================
...
CREATE TYPE submission_route_enum AS ENUM (
    'eudravigilance_evweb',
    'fda_faers_esm',
    'fda_ind_email_paper',
    'pmda_gateway',
    'mhra_yellow_card_api',
    'health_canada_mceyes',
    'tga_daen',
    'anvisa_notivisa',
    'nmpa_adrs_portal',
    'cdsco_sugam',
...
CREATE TABLE health_authority (
    ...
    ha_code         VARCHAR(20)  NOT NULL UNIQUE,   -- e.g. 'FDA', 'EMA', 'PMDA'
    ha_name         VARCHAR(200) NOT NULL,
    country_code    CHAR(2)      NOT NULL,           -- ISO 3166-1 alpha-2
    region          VARCHAR(50),                     -- 'North America', 'Europe', 'APAC', etc.
    ict_region      VARCHAR(20),                     -- ICH / non-ICH
    portal_url      TEXT,
    e2b_version     VARCHAR(10),                     -- 'R2', 'R3', 'both'
    gateway_id      VARCHAR(100),                    -- EV gateway sender ID etc.
...
CREATE TABLE regulatory_framework (
    ...
    regulation_code VARCHAR(100) NOT NULL,           -- e.g. '21 CFR 312.32', 'EU CT Reg 536/2014'
    regulation_name TEXT NOT NULL,
    product_type    product_type_enum[],             -- applicable product types
    phase_scope     development_phase_enum[],        -- applicable phases
    effective_date  DATE,
    sunset_date     DATE,
    source_url      TEXT,
    full_text       TEXT,                            -- cached regulatory text
...
CREATE TABLE reporting_obligation (
    ...
    causality_required  causality_enum[],            -- NULL = any causality triggers

    -- What domestic/foreign sourcing applies
    domestic_foreign    domestic_foreign_enum NOT NULL DEFAULT 'both',

    -- Report details
    report_name         VARCHAR(200) NOT NULL,        -- 'IND Safety Report', 'SUSAR', 'MedWatch 3500A'
    report_type         VARCHAR(50),                  -- '7-day', '15-day', '90-day', 'DSUR', 'PSUR'
    report_format       report_format_enum[],
    submission_route    submission_route_enum[],

    -- Timeline
    timeline_days       INTEGER,                      -- NULL for periodic
    timeline_basis      timeline_basis_enum NOT NULL DEFAULT 'calendar_days',
    clock_start         VARCHAR(200),                 -- 'Day 0 = date of first receipt'
    follow_up_required  BOOLEAN DEFAULT FALSE,
    follow_up_days      INTEGER,                      -- days for follow-up submission

    -- Periodic report linkage
    is_periodic         BOOLEAN NOT NULL DEFAULT FALSE,
    periodic_frequency  VARCHAR(50),                  -- 'Annual', 'Semi-annual', 'Quarterly'
    periodic_report_ref VARCHAR(100),                 -- 'DSUR', 'PSUR', 'PBRER', 'PADER'

    -- Aggregated / line listing
    aggregate_report    BOOLEAN DEFAULT FALSE,

    -- Special conditions
    conditions_text     TEXT,                         -- free text for edge cases
...
CREATE TABLE product_profile (
    ...
    sponsor_id          UUID,                          -- FK to sponsor table
    product_name        VARCHAR(200) NOT NULL,
    inn_name            VARCHAR(200),                  -- INN / generic name
    ...
    ct_gov_nct          VARCHAR(20),                   -- NCT number
    eudra_ct_number     VARCHAR(30),
    isrctn_number       VARCHAR(20),

    -- Approval status by HA
    approved_has        UUID[],                        -- array of ha_ids
...
CREATE TABLE trial_site (
    ...
    pi_name             VARCHAR(200),
    site_status         VARCHAR(50),                  -- 'active', 'closed', 'planned'
...
CREATE TABLE ae_case (
    ...
    argus_case_num      VARCHAR(50),                  -- Argus Safety case number
    ...
    reporter_type           VARCHAR(50),               -- 'HCP', 'Consumer', 'Literature', 'Study'
    is_domestic             BOOLEAN,                   -- domestic to sponsor country?
...
CREATE TABLE submission_record (
    ...
    submitted_date      DATE,
    submission_status   VARCHAR(30) DEFAULT 'pending',  -- 'pending','submitted','accepted','rejected','overdue'
...
-- ─── REFERENCE SAFETY INFO (for expectedness checks) ──────────
CREATE TABLE reference_safety_info (
    rsi_id              UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    product_id          UUID NOT NULL REFERENCES product_profile(product_id),
    document_type       VARCHAR(50) NOT NULL,          -- 'IB', 'SmPC', 'USPI', 'PIL'
    document_version    VARCHAR(30),
    effective_date      DATE,
    listed_reactions    INTEGER[],                     -- array of MedDRA PT codes listed
...
CREATE TABLE ha_feed_log (
    ...
    ha_id               UUID NOT NULL REFERENCES health_authority(ha_id),
    feed_type           VARCHAR(50),                   -- 'regulation_update', 'guidance', 'deadline'
    source_url          TEXT,
...
-- ─── COMMENTS ─────────────────────────────────────────────────
COMMENT ON TABLE reporting_obligation IS 'Core rules engine: maps product/phase/AE attributes to HA-specific reporting requirements';
COMMENT ON TABLE ha_feed_log IS 'Audit trail for automated HA website scraping — change detection for regulatory updates';
COMMENT ON FUNCTION get_reporting_obligations IS 'Primary query function called by the PV Regulatory Agent to resolve obligations';
